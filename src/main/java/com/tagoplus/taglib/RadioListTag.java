...
public class RadioListTag extends AbstractListTag implements DynamicAttributes {
	...
	protected String getHtml() {
		...
		for (int i = 0, s = getListAsList().size(); i < s; i++) {
			...
			//sb.append("<span>");
			sb.append("<div class='custom-control custom-radio custom-control-inline'><input type='radio' class='custom-control-input' id='").append(getId())
			...
			sb.append("/> ").append("<label class='custom-control-label' for='").append(getId()).append('.').append(key).append("'>").append(value).append("</label></div>");
			//sb.append("</span>");
			sb.append("\n");
		}
