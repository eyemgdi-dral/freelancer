...
public class BoardManageController extends BaseController {
	...
	public String boardManageBoardProc(BoardMasterVO vo, BoardSearchVO search, BindingResult result, Model model) throws Exception {
		...
		try {
			...
			if (search.getStatus().toLowerCase().equals("d")) {
				//TODO : 게시판 마스터 삭제
				_boardService.deleteBoardMaster(search, vo);
				sStatus = "삭제";
			}
			else {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/ajax/check_delete", method = RequestMethod.POST)
	@ResponseBody
	public Object ajaxCheckBoardInfo(@RequestBody(required = false) String body, String bcode, BindingResult result, Model model) throws Exception {
		if (result.hasErrors())
			return super.setBindingResult(result, model);
