...
public class PrdManageController extends BaseController {
	...
	public String prdWrite(PrdSearchVO search, BindingResult result, Model model) throws Exception {
		...
		// categorypart
		search.setCateLevel((short) 1);
		@SuppressWarnings("unchecked")
		List<PrdCategoryVO> prdCategoryList = (List<PrdCategoryVO>) _gDao.selectList("prdCategory.selectAll", search);
		
		search.setCateLevel((short)2);
		@SuppressWarnings("unchecked")
		List<PrdCategoryVO> childlist = (List<PrdCategoryVO>) _gDao.selectList("prdCategory.selectAll", search);
		
		search.setCateLevel((short)3);
		@SuppressWarnings("unchecked")
		List<PrdCategoryVO> subChildlist = (List<PrdCategoryVO>) _gDao.selectList("prdCategory.selectAll", search);
		
		PrdCategoryVO voc = (PrdCategoryVO) _gDao.selectOne("prdCategory.selectByKey", vo.getCategory1());
		vo.setCatLevel(voc.getCateLevel());
		
		if (Short.compare(vo.getCatLevel(), (short)1) == 0) {
			vo.setParentCategory(vo.getCategory1());
		}
		if (Short.compare(vo.getCatLevel(), (short)2) == 0) {
			vo.setParentCategory(childlist.stream().filter(x -> vo.getCategory1().equals(x.getCateIdx()))
					.findFirst()
					.orElse(null).getParentIdx());
		}
		if (Short.compare(vo.getCatLevel(), (short)3) == 0) {
			vo.setSubCategory(subChildlist.stream().filter(x -> vo.getCategory1().equals(x.getCateIdx()))
					.findFirst()
					.orElse(null).getParentIdx());
			vo.setParentCategory(childlist.stream().filter(x -> vo.getSubCategory().equals(x.getCateIdx()))
					.findFirst()
					.orElse(null).getParentIdx());
		}
		...
		model.addAttribute("subChildlist", subChildlist);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/ajax/get_category", method = RequestMethod.POST)
	@ResponseBody
	public Object ajaxGetCat(@RequestBody(required = false) String body,String cateSeq, HttpServletRequest request, BindingResult result, Model model) throws Exception {
		...
		try {
			PrdSearchVO search = new PrdSearchVO();
			search.setParentIdx(cateSeq);
			List<PrdCategoryVO> list = (List<PrdCategoryVO>) _gDao.selectList("prdCategory.selectAll", search);		
