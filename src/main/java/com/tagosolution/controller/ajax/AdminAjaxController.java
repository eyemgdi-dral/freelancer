...
public class AdminAjaxController extends BaseController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "#{globals['url.admin.root']}/ajax/getLastMonth.do", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	@ResponseBody
	public Object getLastMonth(@RequestBody(required = false) String body, SearchVO search, BindingResult result,
		...
		try {
			...
			List<HistoryHitVO> historyHitList  = (List<HistoryHitVO>) _dashBoardService.getDashBoardHistoryHitListLastMonth();
			jr.setSuccess(true);
			jr.setData(historyHitList);
		} catch (Exception e) {
			jr.setSuccess(false);
