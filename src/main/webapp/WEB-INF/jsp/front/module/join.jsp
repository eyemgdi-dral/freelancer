				...
				<div class="card bg-secondary border-0 mb-0">
					<div class="card-body px-lg-5 py-lg-5">
						...
						<div class="form-group mb-3 text-center">							
							<button type="button" name="radPCTypeButton" value="P" onclick="doPCTypeCheck(this);" class="btn btn-outline-primary" data-toggle="button" aria-pressed="true">
							개인
							</button>
							<button type="button" name="radPCTypeButton" value="C" onclick="doPCTypeCheck(this);" class="btn btn-outline-primary" data-toggle="button" aria-pressed="false">
							회사
							</button>
						</div>
						<script>
							function doPCTypeCheck(obj) {
								$('button[name=radPCTypeButton]').removeClass('active')
								$('input[name=userType]').val($(obj).val())
								console.log($(obj).val())
								if ($(obj).val() == 'C') {
									$('#SPCompInfo').css('display', 'inherit');
								} else {
	...
	<c:if test="${etcFlag > 0 }"> 
		...
		<c:if test="${etcFlag == 2}">
			...
			<div class="row mt-3">
				...
				<div class="col-6 ">
					<a onclick="javascript:$.Nav('self', {isAgreed: false}, null, true)" class="btn btn-block btn-lg btn-dark">
						<small>취소</small>
					</a>
				</div>
			</div>
					...
					$(function() {
						...
						$('input[name=userId]').change(
								function() {
									...
									if (sUserId != '') {
										if (sUserId.length < 4) {
											$('#userIdCheck').attr('class', 'text-red');
								...
								.change(
										function() {
											...
											if (sUserPwd.length < 8) {
												$('#userPwdCheck').prop('class','text-red');
												$('#userPwdCheck').text('비밀번호는 8~20자 이내의 영문 및 숫자만 입력가능합니다.');
				
											} else {
												$('#userPwdCheck').prop('class','text-blue');
												$('#userPwdCheck').text('비밀번호가 입력되었습니다.');
											}
				
											if (sUserPwd != '' && sUserPwd2 != '') {
												if (sUserPwd != sUserPwd2) {
													$('#userPwdCheck2').prop('class','text-red');
													$('#userPwdCheck2').text('비밀번호가 다릅니다.');
				
												} else {
													$('#userPwdCheck2').prop('class','text-blue');
