...
<div class="container-fluid">
	...
	<form id="form1" action="./board_category_proc" method="post">
		...
		<div class="d-flex my-4 align-items-center justify-content-between">
			...
			<a class="btn btn-primary" onclick="javascript:$.Nav('go', './board_category_write', {bcseq: '', bcode: '${search.bcode}'});">카테고리 등록</a>
		</div>
		
		<table class="table table-neutral table-bordered text-center">
			<colgroup>
...
<c:forEach var="m" items="${list }" varStatus="s">
	...
	<c:if test="${search.mode eq 'move' or search.mode eq 'copy' }">
					...
					<td>${m.cateName }</td>
					<td>
						<a onclick="javascript:$.Nav('go', './board_category_write', {bcseq: '${m.bcSeq}', bcode: '${m.boardCode}'});" class="btn btn-sm btn-primary">수정</a>
						<a onclick="javacript:doDelete('${m.bcSeq}');" class="btn btn-sm btn-danger">삭제</a>
					</td>
				</tr>
</c:forEach>
...
<c:if test="${search.mode eq 'move' }">
			<a onclick="javascript:doSelectBc('move');" class="btn btn-info">이동</a>
</c:if>
<c:if test="${search.mode eq 'copy' }">
			<li><a onclick="javascript:doSelectBc('copy');" class="btn btn-info">복사</a>
</c:if>
		
			<a onclick="javascript:self.close();" class="btn btn-dark">닫기</a>
		</div>
	</form>
</div>
<!--wrap_scon E-->
